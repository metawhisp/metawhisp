    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Glass.s16) {
            ...
            VStack(alignment: .leading, spacing: Glass.s8) {
                ...
                bullet("Reviewed ProjectAlpha backlink report with Alex")
                bullet("Drafted ProjectAlpha Q2 pricing memo")
            }
...
struct TasksScreen: View {
    ...
    private let committed: [(String, Bool, String?)] = [
        ("Check new referral pages on projectalpha.com", false, "Tomorrow"),
        ...
        ScrollView(showsIndicators: false) {
            VStack(spacing: Glass.s12) {
                ...
                bubble("You promised to send a revised doc to Client X by Thursday and to share the updated A/B test link. Both have been captured as Tasks — review them in the Tasks tab.",
                       isUser: false)
                bubble("who is Sam and what does he do", isUser: true)
                bubble("Sam is one of the developers in your network. From memory: he owns the MeetingTool backend and recently fixed an SEO issue on ProjectAlpha. Last mentioned yesterday in the Q2 planning call.",
                       isUser: false)
            }
