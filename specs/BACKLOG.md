# MetaWhisp Backlog — Путь к parity

**Source of truth** для открытых треков. Единственное место где живёт приоритизация.

**Последний deep research:** 2026-04-19 — читал реальный desktop Swift код референса, prompt files, models. Предыдущие оценки scope были критически неполны — извинения user'у записаны.

---

## ⏸ Parked — вернёмся позже

- **Second Brain (сделать приложение «умнее»)** — research + план 2026-06-04. Выбраны 3 направления: умный поиск/память · проактивные напоминания · готовые артефакты. План — в истории сессии.
- **ITER-043 — Напоминания второго мозга** — полное ТЗ: `spec://iterations/ITER-043-reminders`. Аудит подсистемы (855 задач, ≈1% выполнено, скана просрочки нет нигде) + 7 под-итераций под TDD. Ждёт решений D1–D5, старт с 043.1 (чистое ядро + тесты).

---

## Методология (железная)

1. **Analyze reference** — открыть реальный Swift файл референса прежде чем писать строку кода.
...
- Speculation про features которые я не прочитал в repo референса
- Отрицание features которые есть в референсе, без deep search
- Bundle multiple tracks

---

## Reference desktop app — real inventory (2026-04-19 research)

Reference desktop app (`desktop/Desktop/Sources/`) имеет **40+ top-level Swift files + 15 directories**. Это не MVP — это зрелый продукт. Ниже — group by area.
...
### Data models (real field lists code)
...
## Honest gap analysis — MetaWhisp vs reference desktop
...
- **C1.2** ⏸️ TESTING PENDING — Structured generation (title/overview/category/emoji on close). Implementation 2026-04-19:
  ...
  - `Services/Intelligence/StructuredGenerator.swift` — NEW, structured-generation prompt with single-user simplifications. Adaptations: removed speaker/photos/calendar, kept title Title Case ≤10 words + overview + emoji + category (33 values).
...
- **C1.4** ⏸️ TESTING PENDING — Conversations tab UI. Implementation 2026-04-19:
  ...
  - Adaptations : monochrome SF Symbols (not color emoji), simpler 2-chip filter (ALL/STARRED) 's 5+1 (33 categories too many for top bar). Category chip appears inline next to title.
...
### Phase 2: Screen pipeline (Rewind) 🎯 NEXT AFTER PHASE 1
...
Наш ScreenContext (OCR snapshots) — очень bare vs reference Rewind (video chunks + OCR + task/memory extraction per screenshot).

- **R1** ⏸️ TESTING PENDING — ScreenObservation infrastructure + hourly batch extractor. Implementation 2026-04-19:
  - `Models/ScreenObservation.swift` — NEW. Mirrors `Rewind/Core/ObservationRecord.swift`: id, screenContextId FK, appName, windowTitle, contextSummary, currentActivity, hasTask, taskTitle, sourceCategory, focusStatus, startedAt, endedAt, createdAt.
  - `Services/Intelligence/ScreenExtractor.swift` — NEW. Batches ScreenContext into "visits" (consecutive same-app, 5-min gap threshold), caps at 20 visits/batch, sends to Pro proxy. Prompt adapted observation extraction: per-visit contextSummary + currentActivity + hasTask + category + focusStatus.
  ...
  - **Adaptations :** per-snapshot (cheap because wearable captures slow); we batch to fit Pro-proxy economics. Output shape identical (observation + hasTask + taskTitle).
...
- **R3** ⏸️ TESTING PENDING — Rewind timeline UI. Implementation 2026-04-19:
  ...
  - Adaptations : no video scrubber (OCR-only corpus), no draggable timeline seek — list + expand is enough for desktop.
...
- **R4** (large) Video chunk storage — рассмотреть. Reference пишет video, мы только screenshot. Skip for MVP, revisit.

### Phase 3: External readers (Apple Notes / Files / Calendar / Gmail)

External readers — самый близкий аналог "auto-подтягивать memories из Obsidian/Notion" что user хочет.

- **E1** ⏸️ TESTING PENDING — File Indexing + memory extraction from user-picked folders. Implementation 2026-04-19:
  - `Models/IndexedFile.swift` — mirrors `IndexedFileRecord` (path, filename, fileExtension, fileType, sizeBytes, folder, depth, createdAt/modifiedAt/indexedAt/contentExtractedAt). Static helpers: category by ext (10 types), isExtractable(md/txt/rtf/markdown).
  - `Services/Indexing/FileIndexerService.swift`, skip-folder list (.git/node_modules/.venv/...), package-extensions (.app/.framework) as leaves, maxDepth=8 (user vaults deeper than 3), maxFileSize=500MB, batch save every 200. Periodic every 6h.
  - `Services/Indexing/FileMemoryExtractor.swift` — second pass: reads .md/.txt content (up to 12K chars), sends to Pro proxy with strict memory prompt, saves UserMemory with `sourceFile` FK. Max 15 files per run, confidence ≥ 0.7.
  ...
  - **Adaptation note:** user explicitly asked only user-picked folders (Obsidian vault), no auto-scan of Downloads/Documents. We diverge 's onboarding pipeline accordingly.
...
- **E2** ⏸️ TESTING PENDING — Apple Notes reader + memory extraction. Implementation 2026-04-19:
  ...
  - **Adaptation :** NoteStore.sqlite directly via GRDB (requires Full Disk Access); we use AppleScript (requires Automation permission prompt, simpler, full body access). Tradeoff: dependent on Notes.app running; slightly slower than SQLite but no new dependencies.
...
- **E3** ⏸️ TESTING PENDING — Calendar reader via EventKit. Implementation 2026-04-19:
  - `Services/Indexing/CalendarReaderService.swift` — NEW. EventKit-based: requestFullAccessToEvents (macOS 14+), predicateForEvents window=-30d…+14d. For each upcoming non-cancelled non-declined event → create TaskItem with dueAt=startDate + sourceApp="Calendar". Dedup by (title.lowercase, hourBucket). Also sends last 80 events to LLM for pattern memories ("User has weekly 1-on-1 with Alex on Tuesdays"), sourceApp="Calendar" + sourceFile="calendar".
...
- **KG4** Graph visualization UI (force-directed layout)
...
**Deferred to Phase 6+:** premium cloud TTS (OpenAI / ElevenLabs for premium "Sloane" voice), floating bar UI, streaming transcription, wake-word, voice commands.
...
- **P3** Proactive suggestions engine ("Next step" card on Home)
...
- **A1** Claude OAuth provider ("Your Claude Account") — copy `ClaudeAuthSheet`
- **A2** Workspace selector (project directory for chat context)
- **A3** Agent-on-task (Claude processes task description → does work). tmux integration copy
...
3. **Before coding:** open corresponding reference Swift file, read implementation. Reflect in scope proposal as "Reference: `path/to/file.swift:NNN`".
