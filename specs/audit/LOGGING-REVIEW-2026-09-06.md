...
## Статус P1 — закрыто 2026-09-08

Все 13 P1 разобраны: 9 починены с падающим тестом первым, 4 оказались
дубликатами (один и тот же дефект нашли две фичи).

| Дефект | Что сделано | Тест |
|---|---|---|
| Выход из приложения теряет встречу | `applicationShouldTerminate` спасает обе дорожки в Recovery, ожидание ограничено 6 с | `MeetingShutdownTests` |
| «Движок не готов» выбрасывает аудио | обе дорожки пишутся в Recovery, баннер называет папку | `MeetingShutdownTests` |
| `historyService.save` вернул nil → «✅» | транскрипт пишется в Recovery, баннер называет файл, лог не врёт | `MeetingShutdownTests` |
| `regenerate()` стирает 11 полей до генерации | ничего не стирается, полный путь запрашивается явно | `StructuredRegenerateTests` |
| Локальная свёртка молча теряет кусок | `fold()` отдаёт текст вместе с числом пропусков | `ChunkedCompletionTests` |
| Подтверждение памяти голосом не сохранялось | чтение и запись в одном контексте | `MemoryProposalConfirmationTests` |
| Горячие клавиши мертвы после поздней выдачи доступа | перерегистрация по факту выдачи, потолок 10 мин | `HotkeyRearmTests` |
| Список фактов вытеснял транскрипт из промпта | доля 3/10 на всех путях, честное «омитировано N» | `MemoryPromptBudgetTests` |
| Транскрипт ждал сетевого списания квоты | сохранение раньше биллинга | по инспекции (нет шва) |
| Неудачный перевод молчал | карточка с причиной | `SelectionTranslateFeedbackTests` |
| Перевод уничтожал буфер обмена | займ и полный возврат | `SelectionTranslateClipboardTests` (1.3.30) |

Открытым остаётся P2 (55 штук) и вопрос про названия проектов в логах
`ProjectAggregator`.
