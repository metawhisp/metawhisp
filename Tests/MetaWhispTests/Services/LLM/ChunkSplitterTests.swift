...
final class ChunkSplitterTests: XCTestCase {
    ...
    func testShortInputSingleChunk() {
        let out = ChunkedCompletion.splitBySentences("Привет. Как дела?", limit: 100)
    ...
    func testNoCharactersLostAndLimitsRespected() {
        ...
        let chunks = ChunkedCompletion.splitBySentences(text, limit: 500)
    ...
    func testMixedBoundaries() {
        ...
        let chunks = ChunkedCompletion.splitBySentences(text, limit: 300)
        XCTAssertEqual(chunks.joined(), text)
